                ...
                var cantidadcom=0;
    				...
    				if($.trim(productoImpuestosIndexes) != '' && $.trim(productoImpuestosIndexes) != 0){
    					...
    					if($.trim(productoImpuestosIndexes).indexOf('@') !== -1){
    						...
    						if($('#impuestoFactura-'+productoImpuestosIndexes).length == 0){
    							var impuestoDetalles = $('#impuesto-'+ productoImpuestosIndexes).val().split('|');
								var impcalc=((parseFloat(productoCantidad) * (parseFloat(productoPrecio)+sumaagregados)) * parseFloat(impuestoDetalles[2]));
    							taxTotal+=impcalc;
                                //alert('si'+taxTotal);
    							$('#factura').append('<input type="hidden" id="impuestoFactura-'+ productoImpuestosIndexes +'" class="esImpuesto" data-id="'+ impuestoDetalles[0] +'" data-nombre="'+ impuestoDetalles[1] +'" data-valor="'+ impuestoDetalles[2] +'" value="'+ impcalc +'"/>');
    							}
    						else{
    							var impuestoDetalles = $('#impuesto-'+ productoImpuestosIndexes).val().split('|');
    							var currentTax = $('#impuestoFactura-'+ productoImpuestosIndexes).val();
								var impcalc=((parseFloat(productoCantidad) * (parseFloat(productoPrecio)+sumaagregados)) * parseFloat(impuestoDetalles[2]));
    							taxTotal+=impcalc;
    							$('#impuestoFactura-'+ productoImpuestosIndexes).val(parseFloat(currentTax) + parseFloat(impcalc));
    							}
    						}
    					}else{
    						subtotalSinIvaCompra=(parseFloat(productoCantidad) * (parseFloat(productoPrecio)+sumaagregados));
